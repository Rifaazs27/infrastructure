...
Ce guide permet de vérifier que toutes les exigences techniques du
projet CloudShop sont respectées.

------------------------------------------------------------------------

## 1. Namespace

**Objectif** : Vérifier que le namespace dédié existe et que rien n'est
dans default.
...
✅ Vérifier que cloudshop existe et que default ne contient aucun objet
du projet.
...
✅ Vérifier que les pods backend se mettent à jour sans interruption du
service.
...
**Objectif** : Vérifier les types de service et l'exposition via
Ingress.
...
✅ Vérifier que le frontend et le backend sont accessibles via les
chemins `/` et `/api`.
...
Puis supprimer le pod postgres et vérifier que les données persistent.
...
✅ Vérifier : - Node DB a le taint `db-node=true:NoSchedule` - DB pod
tourne sur ce node - Backend pods sur nodes différents - Frontend et DB
ne cohabitent pas
...
✅ Vérifier : - Backend peut lire ConfigMaps et Secrets - Frontend n'a
aucun droit API
...
kubectl exec -it <frontend-pod> -- curl http://backend:5000/api
kubectl exec -it <frontend-pod> -- psql -h postgres
kubectl exec -it <other-namespace-pod> -- curl http://backend:5000/api
```

✅ Vérifier : - Frontend -\> Backend autorisé - Backend -\> DB
autorisé - Frontend -\> DB interdit - Pods externes -\> Backend/DB
interdits
